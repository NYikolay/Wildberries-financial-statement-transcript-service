...
def handle_incorrect_report_obj(current_user, report_data: dict, current_api_key):
...
def generate_incorrect_reports(current_user, incorrect_reports_data_list: list, current_api_key) -> None:
    """
    ...
    for incorrect_report in incorrect_reports_data_list:
        if incorrect_report.get('realizationreport_id') not in generated_incorrect_reports:
            incorrect_reports_objs_list.append(handle_incorrect_report_obj(
                current_user,
                incorrect_report,
                current_api_key
            ))
