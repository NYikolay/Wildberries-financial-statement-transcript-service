...
def get_wb_request_response(date_from: str, date_to: str, current_api_key) -> dict:
    """
    ...
    unique_report_ids: List[int] = list(get_unique_reports(response.get('data')))
    ...
    handled_response_data['status'] = response.get('status')
    handled_response_data['data'] = [
        sale for sale in response.get(
            'data'
