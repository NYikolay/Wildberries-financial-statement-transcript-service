...
def create_reports_additional_data(file, current_api_key):
    ...
    for realizationreport_id, report_object in current_user_reports.items():
        ...
        if all([isinstance(val, (int, float)) for val in [storage_cost, cost_paid_acceptance, other_deductions]]):
            report_object.storage_cost = report_object.storage_cost or storage_cost
            report_object.cost_paid_acceptance = report_object.cost_paid_acceptance or cost_paid_acceptance
            report_object.other_deductions = report_object.other_deductions or other_deductions
